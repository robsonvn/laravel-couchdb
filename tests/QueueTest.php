...
class QueueTest extends TestCase
{
    ...
    {
        list($id,$rev) = Queue::push('test', ['action' => 'QueueJobLifeCycle'], 'test');
        $this->assertNotNull($id);

        // Get and reserve the test job (next available)
        $job = Queue::pop('test');
        $this->assertInstanceOf(Robsonvn\CouchDB\Queue\CouchJob::class, $job);
    ...
    {
        list($id,$rev) = Queue::push('test', ['action' => 'QueueJobExpired'], 'test');
